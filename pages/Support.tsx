...
import { Role, SystemSettings, DiagnosticResult, AppTheme, Notification } from '../types';
import { Activity, CheckCircle, Server, RefreshCw, Power, Database, Cloud, HardDrive, ToggleLeft, ToggleRight, Save, ShieldCheck, AlertTriangle, XCircle, Megaphone, Send, Loader2, Image, Palette, Check, X, Sun, Moon, Radio } from 'lucide-react';
...
const Support: React.FC = () => {
  ...
  // Broadcast Form
  const [broadcast, setBroadcast] = useState({ role: 'all', message: '', isPopup: false });
  ...
  const handleBroadcast = async () => {
      ...
      const notification: Notification = {
          ...
          title: 'تنبيه من الدعم الفني',
          message: broadcast.message,
          type: broadcast.isPopup ? 'announcement' : 'general',
          isPopup: broadcast.isPopup,
          target_audience: broadcast.role as any,
              ...
              <div className="glass-card p-8 rounded-[2rem] border border-white/10">
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                      <div className="w-full md:w-1/3 space-y-4">
                          ...
                          {/* Popup Toggle */}
                          <div className="p-4 bg-white/5 rounded-xl border border-white/5 flex items-center justify-between cursor-pointer hover:bg-white/10" onClick={() => setBroadcast({...broadcast, isPopup: !broadcast.isPopup})}>
                              <div className="text-xs text-gray-300">إعلان منبثق (Popup)</div>
                              <div className={`w-10 h-6 rounded-full p-1 transition-colors ${broadcast.isPopup ? 'bg-primary-500' : 'bg-gray-600'}`}>
                                  <div className={`w-4 h-4 rounded-full bg-white transition-transform ${broadcast.isPopup ? 'translate-x-4' : ''}`}></div>
                              </div>
                          </div>

                          <div className="text-[10px] text-gray-500 leading-relaxed px-1">
                              الإعلانات المنبثقة ستظهر كنافذة عائمة للمستخدمين بمجرد دخولهم.
                          </div>
                      </div>

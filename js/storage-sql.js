...
const DEFAULT_CUSTOM_THEME = Object.freeze({ background: "#0b1320", strong: "#2f80ed", muted: "#a7b2c7" });
  ...
  function normalizeCustomThemeColors(source) {
    ...
    const strong = sanitizeHexColor(source?.strong ?? source?.customThemeTextStrong, defaults.strong);
    const muted = sanitizeHexColor(source?.muted ?? source?.customThemeTextMuted, defaults.muted);
    return {
      background,
      strong,
      muted,

        ...
        <>
           ...
           {isEditing && (
             <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10 rounded-2xl">
                <div className="flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-lg text-gray-200">جاري تطبيق التعديلات...</p>
                </div>
            </div>
          )}
          <div className="absolute bottom-4 left-4 right-4 flex flex-col items-center gap-3">
             <div className="flex justify-center items-center gap-2 flex-wrap">
                ...
                <span>إعادة إنشاء بجودة فائقة</span>
                </button>
             </div>
            <PostGenerationEditor onEdit={onEditImage} disabled={isEditing || isLoading} />
          </div>
        </>
      ) : (
        <Placeholder />
...
function App(): React.ReactElement {
  ...
  const handleGenerateHQ = useCallback(async () => {
    if (imageFiles.length === 0 || !prompt.trim()) {
      setError('لا يمكن تحسين الجودة بدون صور ووصف.');
      return;
    }
    setError(null);
    setIsLoading(true);
    ...
    const enhancedPrompt = `${prompt.trim()}، جودة فائقة، واقعي جداً، تركيز حاد، 8k، تصوير احترافي`;

    try {
      const files = imageFiles.map(img => img.file);
      const newImage = await generateProductImage(files, enhancedPrompt);
      const imageUrl = `data:image/jpeg;base64,${newImage}`;
      setGeneratedImage(imageUrl);
      setEditHistory([imageUrl]);
      setHistoryIndex(0);
      ...
      <main className="flex-grow container mx-auto p-4 md:p-8 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          <div className="flex flex-col gap-6 p-6 bg-gray-800/50 rounded-2xl shadow-lg border border-gray-700">
            ...
            >
              {isLoading ? 'جاري إنشاء المشهد...' : '✨ إنشاء مشهد احترافي'}
            </button>
          </div>
          <ResultDisplay
            isLoading={isLoading}

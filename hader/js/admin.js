...
(function () {
  ...
  function bindLogoutButton() {
    ...
    if (lb && !lb.dataset._b) {
      lb.addEventListener('click', () => {
        if (typeof window.haderLogout === 'function') {
          window.haderLogout();
        }
      });
      lb.dataset._b = '1';
  ...
  function showSkeletonRows(tbody, count = 8) {
    if (!tbody) {
      return;
    }
  ...
  function buildStudentsListSubtab(panel, subpanel) {
    ...
    if (!subpanel.dataset._built) {
      ...
      emptyState.dataset.role = 'students-empty';
      emptyState.textContent = 'لا توجد بيانات لعرضها حالياً.';
      emptyState.hidden = true;

      card.append(toolbar, tableWrap, emptyState);
      grid.appendChild(card);
      subpanel.appendChild(grid);
    ...
    const emptyState = subpanel.querySelector('[data-role="students-empty"]');
    ...
    const applyFilters = () => {
      ...
      renderTableRows(tableBody, filteredRows, STUDENT_TABLE_COLUMNS);
      if (emptyState) {
    ...
    if (exportButton && !exportButton.dataset.bound) {
      exportButton.addEventListener('click', () => {
        ...
        try {
          exportToCSV(filteredRows, STUDENT_TABLE_COLUMNS, createTimestampedFilename('students', 'csv'));
        } catch (error) {
  ...
  function openStudentFormModal(options = {}) {
    ...
    phoneField.innerHTML = `
      ...
      <input type="tel" class="input" name="student-phone" placeholder="05xxxxxxxx" autocomplete="tel" />
    `;
    ...
    form.addEventListener('submit', (event) => {
      event.preventDefault();
      const payload = {
        name: form.querySelector('[name="student-name"]').value.trim(),
        grade: gradeSelect.value,
        classroom: classSelect.value,
        guardian: form.querySelector('[name="student-guardian"]').value.trim(),
        phone: form.querySelector('[name="student-phone"]').value.trim()
      };
      if (!payload.name) {
      ...
      timingCard.innerHTML = `
        ...
        <div class="field-grid">
          <label class="field">
            ...
            <input type="time" class="input" data-role="timing-assembly" />
          </label>
          <label class="field">
            <span>دقائق السماح</span>
            <input type="number" class="input" data-role="timing-grace" min="0" step="1" placeholder="0" />
          </label>
          <label class="field">
            <span>عتبة التأخير بالدقائق</span>
            <input type="number" class="input" data-role="timing-late" min="0" step="1" placeholder="0" />
          </label>
        </div>
        <div class="form-actions">
  ...
  function buildSettingsRemoteSubtab(panel, subpanel) {
    buildWipCard(subpanel, 'settings-remote');
  }
  ...
  function buildAttendanceDailyLogSubtab(panel, subpanel) {
    ...
    if (!subpanel.dataset._built) {
      ...
      emptyState.className = 'attendance-table__empty';
      emptyState.dataset.role = 'attendance-empty';
      emptyState.textContent = 'لا توجد سجلات مطابقة للمعايير الحالية.';
      emptyState.hidden = true;

      card.append(tableWrap, emptyState);
      grid.appendChild(card);
      subpanel.appendChild(grid);
      subpanel.dataset._built = '1';
    }
    ...
    const emptyState = subpanel.querySelector('[data-role="attendance-empty"]');
    ...
    const updateTable = async () => {
      ...
      try {
        ...
        renderTableRows(tableBody, Array.isArray(rows) ? rows : [], ATTENDANCE_TABLE_COLUMNS);
        if (emptyState) {
          emptyState.hidden = Array.isArray(rows) && rows.length > 0;
        }
      } catch (error) {
        console.error('تعذّر تحميل سجل الحضور.', error);
        renderTableRows(tableBody, [], ATTENDANCE_TABLE_COLUMNS);
        if (emptyState) {
  ...
  function createErrorLogCard() {
    ...
    card.innerHTML = `
      <h3>السجل والدعم</h3>
      <p class="muted">يعرض هذا السجل آخر الأخطاء التي تم رصدها تلقائيًا (حتى ٥٠ حدثًا).</p>
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              <th scope="col">الوقت</th>
              <th scope="col">النوع / الرسالة</th>
              <th scope="col">التفاصيل</th>
            </tr>
          </thead>
          <tbody data-role="error-log-body">
            <tr>
              <td colspan="3" class="muted">لا توجد أخطاء مسجّلة.</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="form-actions" style="margin-top:12px;">
        <button type="button" class="btn" data-action="error-log-refresh">تحديث</button>
        <button type="button" class="btn" data-action="error-log-clear">تنظيف السجل</button>
      </div>
    `;

    const tbody = card.querySelector('[data-role="error-log-body"]');
    const refreshButton = card.querySelector('[data-action="error-log-refresh"]');
    const clearButton = card.querySelector('[data-action="error-log-clear"]');

    const update = (entries) => {
      renderErrorLogRows(tbody, entries);
    };

    const handleUpdate = (event) => {
      const entries = Array.isArray(event?.detail)
        ? event.detail
        : readJSON(ERROR_LOG_STORAGE_KEY, []);
      update(entries);
    };

    window.addEventListener(ERROR_LOG_EVENT, handleUpdate);

    if (refreshButton) {
      refreshButton.addEventListener('click', () => {
        update(readJSON(ERROR_LOG_STORAGE_KEY, []));
      });
    }
    ...
    if (clearButton) {
      clearButton.addEventListener('click', () => {
        ...
        showToast('تم تنظيف سجل الأخطاء.', 'success');
      });
    }
  ...
  function createWipCard(areaKey) {
    const card = document.createElement('div');
    card.className = 'card';
    card.innerHTML = `
      <h3>تحت التطوير</h3>
      <p class="muted">هذه الشاشة قيد التطوير ضمن خارطة الطريق القادمة.</p>
      <div style="display:flex; gap:8px; margin-top:8px; justify-content:flex-end;">
        <button class="btn" data-role="suggest">اقتراح ميزة</button>
      </div>`;

    const button = card.querySelector('[data-role="suggest"]');
    if (button) {
      button.addEventListener('click', () => {
        const dialog = document.createElement('dialog');
        ...
        dialog.innerHTML = `
          <div class="form-row">
            <input class="input" type="text" data-role="wip-suggest" placeholder="اكتب اقتراحك المختصر هنا">
          </div>
        ...
        const input = dialog.querySelector('[data-role="wip-suggest"]');
        ...
        if (saveButton) {
          saveButton.addEventListener('click', () => {
            ...
            if (!value) {
              showToast('يرجى كتابة اقتراح مختصر أولاً.', 'error');
            ...
            suggestions.push({ area: areaKey || 'unknown', text: value, ts: Date.now() });
            writeJSON(WIP_SUGGESTIONS_STORAGE_KEY, suggestions);
            dialog.close('submit');
            showToast('تم إرسال الاقتراح', 'success');
          });
        }
  ...
  function buildWipCard(subpanel, areaKey) {
    if (!subpanel || subpanel.dataset._built) {
      return;
    }
    ...
    const grid = document.createElement('div');
    grid.className = 'cards-grid';
    grid.appendChild(createWipCard(areaKey));
    subpanel.appendChild(grid);
    subpanel.dataset._built = '1';
  }
  ...
  function buildHelpWipSubtab(panel, subpanel) {
    ...
    const grid = document.createElement('div');
    grid.className = 'cards-grid';
    grid.appendChild(createWipCard('help'));
    grid.appendChild(createErrorLogCard());
    subpanel.appendChild(grid);
    subpanel.dataset._built = '1';
  }

  function buildAboutWipSubtab(panel, subpanel) {
    buildWipCard(subpanel, 'about');
  }

  function buildSmartToolsWipSubtab(panel, subpanel) {
    buildWipCard(subpanel, 'smart-tools');
  }

...
import { Scan, Sparkles, Loader2, Wifi, Clock, Calendar, AlertCircle, CheckCircle, Users } from 'lucide-react';
...
const Kiosk: React.FC = () => {
  ...
  useEffect(() => {
      ...
      db.getSettings().then(s => {
          if (s.kiosk) setSettings(s.kiosk);
          // Initial Fetch stats if enabled
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Update live stats locally for instant feedback
    if (result.success && settings.showStats && liveStats) {
        // If not already counted (simple increment, though fetchStats refreshes accurately)
        // We'll just re-fetch for accuracy
        await fetchStats(); 
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 overflow-hidden relative bg-[#0f172a]" onClick={() => inputRef.current?.focus()}>
      ...
      <div className="w-full max-w-5xl text-center space-y-8 relative z-10 flex flex-col items-center">
        ...
        {/* Info Panel (Persistent for 2 mins) */}
